...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (do_fill){
            ...
            // COLLECT STATS
            //Arohan
            if((cache_type == IS_L2C) && (MSHR.entry[mshr_index].instruction == 1) || (cache_type == IS_L1I)){
                //cout << " Hello " << block[set][way].ip << endl;
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                if (do_fill) {
                    ...
                    // COLLECT STATS
                    //Arohan
                    if((cache_type == IS_L2C) && (WQ.entry[index].instruction == 1) || (cache_type == IS_L1I)){
                        if(block[set][way].ip>18000000000000000000){
                            if(WQ.entry[index].ip>18000000000000000000){
