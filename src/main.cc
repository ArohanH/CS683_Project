...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    //Arohan
    cout << " Total number of instructions: " << num_instrs << endl;
    uint64_t num_kernel_instrs = ooo_cpu[cpu].num_of_instrs_kernel_mode - ooo_cpu[cpu].begin_num_of_instrs_kernel_mode;
    uint64_t num_user_instrs = ooo_cpu[cpu].num_of_instrs_user_mode - ooo_cpu[cpu].begin_num_of_instrs_user_mode;
    if(cache -> cache_type == IS_L1I || cache -> cache_type == IS_L2C){
        cout << cache->NAME;
        cout << " Total Kernel Mode Instructions: " << num_kernel_instrs << " kernel misses: " << cache->num_of_sim_miss_in_kernel_mode[cpu][0]  << " and MPKI: " << ((double)cache->num_of_sim_miss_in_kernel_mode[cpu][0]*1000/num_kernel_instrs) << endl;
        cout << " Total User Mode Instructions: " << num_user_instrs << " user misses: " << cache->num_of_sim_miss_in_user_mode[cpu][0] << " and MPKI: " << ((double)cache->num_of_sim_miss_in_user_mode[cpu][0]*1000/num_user_instrs) << endl;
    }
...
void print_sim_stats(uint32_t cpu, CACHE *cache)
{
    ...
    if(cache -> cache_type == IS_L1I || cache -> cache_type == IS_L2C){
        cout << cache->NAME;
        cout << " Total Kernel Mode Instructions: " << num_kernel_instrs << " and MPKI: " << ((double)cache->num_of_sim_miss_in_kernel_mode[cpu][0]*1000/num_kernel_instrs) << endl;
        cout << " Total User Mode Instructions: " << num_user_instrs << " and MPKI: " << ((double)cache->num_of_sim_miss_in_user_mode[cpu][0]*1000/num_user_instrs) << endl;
    }
    //Arohan
...
void reset_cache_stats(uint32_t cpu, CACHE *cache)
{
	...
	cache->sim_instr_miss[cpu][i] = 0;
        //Arohan
        cache->num_of_sim_miss_in_kernel_mode[cpu][i]=0;
        cache->num_of_sim_miss_in_user_mode[cpu][i]=0;
        cache->num_of_replacements_by_kernel_to_victim_kernel[cpu]=0;
        cache->num_of_replacements_by_kernel_to_victim_user[cpu]=0;
        cache->num_of_replacements_by_user_to_victim_kernel[cpu]=0;
        cache->num_of_replacements_by_user_to_victim_user[cpu]=0;
        //Arohan
    }
...
void finish_warmup()
{
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        ...
        //Arohan 
        ooo_cpu[i].begin_num_of_instrs_kernel_mode = ooo_cpu[i].num_of_instrs_kernel_mode;
        ooo_cpu[i].begin_num_of_instrs_user_mode = ooo_cpu[i].num_of_instrs_user_mode;
        //Arohan
