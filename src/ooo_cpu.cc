...
//Neelu: Addition begin
		...
		if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE) {
            ...
            //Arohan Hazarika
            //outfile << "The instruction id of current instruction fetched is " << arch_instr.instr_id << " and the instruction pointer (PC) is " << arch_instr.ip << " " << arch_instr.is_branch << std::endl;
            //Arohan Hazarika
...
//Neelu: Addition end
	  ...
	  {
        //Arohan Comments
        //This else block is not for knob_cloudsuite
        //Arohan Comments
	  	//DP( if (warmup_complete[cpu]) {
		...
		input_instr trace_read_instr;
		    ...
		    else
		    {
                ...
                //Arohan Hazarika
                //cout << "The instruction " << (uint64_t)arch_instr.is_branch << " "<< (uint64_t)arch_instr.branch_taken<< std::endl;
                //Arohan Hazarika
...
void O3_CPU::retire_rob()
{
    for (uint32_t n=0; n<RETIRE_WIDTH; n++) {
        ...
        ooo_model_instr empty_entry;
        //Arohan
        if(ROB.entry[ROB.head].ip>18000000000000000000)
            num_of_instrs_kernel_mode++;
        else num_of_instrs_user_mode++;
        //Arohan
