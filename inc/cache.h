...
class CACHE : public MEMORY {
  public:
    ...
    uint64_t num_of_replacements_by_kernel_to_victim_user[NUM_CPUS], // All of them are to track evictions (who evicts whom)
             ...
             num_of_replacements_by_user_to_victim_kernel[NUM_CPUS];
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8) 
        ...
        for (uint32_t i=0; i<NUM_CPUS; i++) {
            ...
            //Arohan
            num_of_replacements_by_kernel_to_victim_kernel[i]=0;
            num_of_replacements_by_kernel_to_victim_user[i]=0;
            num_of_replacements_by_user_to_victim_kernel[i]=0;
            num_of_replacements_by_user_to_victim_user[i]=0;
            //Arohan
            for (uint32_t j=0; j<NUM_TYPES; j++) {
                ...
                sim_miss[i][j] = 0;
                // 
                num_of_sim_miss_in_user_mode[i][j]=0;
                num_of_sim_miss_in_kernel_mode[i][j]=0;
                //Arohan
